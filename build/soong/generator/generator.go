...
func init() {
	android.RegisterModuleType("prototype_generator", GeneratorFactory)
...
func (g *Module) GenerateAndroidBuildActions(ctx android.ModuleContext) {
	...
	if depRoot == "" {
		...
		depRoot = prototypeExpandVariables(ctx, depRoot)
	}

	// Glob dep_files property
	for _, dep_file := range g.properties.Dep_files {
		dep_file = prototypeExpandVariables(ctx, dep_file)
	...
	cmd := prototypeExpandVariables(ctx, String(g.properties.Cmd))
