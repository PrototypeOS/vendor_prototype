function __print_prototype_functions_help() {
cat <<EOF
Additional PrototypeOS functions:
...
- githubremote:    Add git remote for PrototypeOS Github.
...
function breakfast()
{
    ...
    if [ $# -eq 0 ]; then
        ...
        if [ $? -eq 0 ]; then
            ...
            # This is probably just the PROTOTYPE model name
            if [ -z "$variant" ]; then
                variant="userdebug"
            fi

            lunch prototype_$target-$variant
        fi
    fi
    return $?
}
...
function prototyperemote()
{
    ...
    git remote rm prototype 2> /dev/null
    ...
    then
        ...
        local PFX="PrototypeOS/"
    else
        local PROJECT=$REMOTE
    fi

    local PROTOTYPE_USER=$(git config --get review.review.prototype.org.username)
    if [ -z "$PROTOTYPE_USER" ]
    then
        git remote add prototype ssh://review.prototype.org:29418/$PFX$PROJECT
    else
        git remote add prototype ssh://$PROTOTYPE_USER@review.prototype.org:29418/$PFX$PROJECT
    fi
    echo "Remote 'prototype' created"
}
...
function githubremote()
{
    ...
    git remote add github https://github.com/PrototypeOS/$PROJECT
...
function makerecipe() {
    ...
    then
        pwd
        prototyperemote
        git push prototype HEAD:refs/heads/'$1'
    fi
    '
}

function prototypegerrit() {
    ...
    local user=`git config --get review.review.prototype.org.username`
...
EOF
            ...
            case $1 in
                ...
                changes|for)
                    if [ "$FUNCNAME" = "prototypegerrit" ]; then
...
function prototyperebase() {
    ...
    if [ -z $repo ] || [ -z $refs ]; then
        echo "PrototypeOS Gerrit Rebase Usage: "
        echo "      prototyperebase <path to project> <patch IDs on Gerrit>"
    ...
    git fetch "http://review.prototype.org/p/$repo" "refs/changes/$refs" && git cherry-pick FETCH_HEAD
    if [ "$?" != "0" ]; then
...
function cmka() {
    if [ ! -z "$1" ]; then
        for i in "$@"; do
            case $i in
                prototype|otapackage|bacon|systemimage)
                    mka installclean
                    mka $i
                    ;;
                *)
...
function repopick() {
    T=$(gettop)
    $T/vendor/prototype/build/tools/repopick.py $@
}
function fixup_common_out_dir() {
    common_out_dir=$(get_build_var OUT_DIR)/target/common
    target_device=$(get_build_var TARGET_DEVICE)
    common_target_out=common-${target_device}
    if [ ! -z $PROTOTYPE_FIXUP_COMMON_OUT ]; then
